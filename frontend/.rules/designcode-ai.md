...
>
> **审查规则**：每次代码改动完成后，必须基于对项目的全局理解，主动回复当前操作是否存在问题（一致性、副作用、遗漏项、更优方案），如无问题也需明确告知。
...
6. **Store 文件**: 使用 camelCase，如 `common.js`、`user.js`
7. **Middleware 文件**: `NN.name.global.js` 编号格式，如 `01.login.global.js`

## 二、Props / Emits 命名规范（核心）

**全部使用驼峰命名（camelCase），绝对禁止短横线（kebab-case）。**
...
defineEmits(['close', 'loginSuccess', 'editDesc', 'clearAll', 'updateModelValue'])
```
...
1. 所有组件**必须使用 `<script setup>`** 语法
2. 样式使用 `<style scoped>`（SCSS 需加 `lang="scss"`）
3. v-model 双向绑定使用 `defineProps('modelValue')` + `emit('update:modelValue')`
4. 事件名使用 camelCase：`@loginSuccess`，不是 `@login-success`
5. **直接解构 `useGeneration()`**，不要用中间变量：`const { isLogin, taskStatus } = useGeneration()`
6. **`storeToRefs` 解构的值是 ref**，在 `<script>` 中必须用 `.value` 访问（只有 `<template>` 才自动解包）

### ✅ 正确
```js
const { isLogin, credits } = storeToRefs(userStore)
// <script> 中必须 .value
if (!isLogin.value) { return }
if (credits.value < count) { return }
...
if (!isLogin.value) { openLoginModal(); return }
```
...
if (!gen.isLogin.value) { }  // gen 多余

// <script> 中永远不能省略 .value
if (!isLogin) { return }        // isLogin 是 RefImpl 对象，始终 truthy
if (credits < count) { return } // credits 是 RefImpl，数字比较始终为 false
```
...
4. Composable 返回的对象在组件中先赋值再解构使用
